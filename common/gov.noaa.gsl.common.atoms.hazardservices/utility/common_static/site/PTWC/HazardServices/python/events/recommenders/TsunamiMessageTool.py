...
class Recommender(TsunamiRecommenderCommon.TsunamiRecommenderCommon):
    ...
    def defineScriptMetadata(self):
        ...
        '''
        metaDict = {
            "toolName": "TsunamiMessageTool (PTWC)",
            "author": "GSL",
            "version": "1.0",
    ...
    def defineDialog(self, eventSet, **kwargs):
        ...
        # Create ThreatMessage
        choices = self.getTsunamiThreatMessageRegionChoices()
        tsunamiThreatMessageDict = {
            "fieldType": "Group",
            "fieldName": "tstmGroup",
            "fields": [
               {
                "fieldType": "CheckBoxes",
                "fieldName": "tsuThreatMessage",
                "label": "Create TS.ThreatMessage for:",
                "choices": choices,
                "values": []
                }
               ]
            }
        dialogDict["fields"].append(tsunamiThreatMessageDict)
    ...
    def execute(self, eventSet, dialogInputMap, visualFeatures):
        ...
        # Create ThreatMsgs for PTWC if there is selected region
        threatMessageRegions = dialogInputMap.get("tsuThreatMessage")
        if len(threatMessageRegions) > 0:
            proposedNewNonWWAEventSet = self.createTsunamiMessages("TS.ThreatMessage", threatMessageRegions, proposedNewNonWWAEventSet)
    ...
    def getTsunamiThreatMessageRegionChoices(self):
        return [
            {"identifier": "Pac", "displayString": "Non U.S.Pacific"},
            {"identifier": "Car", "displayString": "Non U.S.Caribbean"},
            ]
