...
/**
 * Plotter / renderer for Unknown physical events.
 *
 * <pre>
 *
 * SOFTWARE HISTORY
 ...
 *
 */
public class UnknownPlotter extends GeneralPEPlotter {
    ...
    private static final RGB DOT_COLOR = new RGB(255, 255, 255);

    private static final double SELECT_EVENT_CIRCLE_SCREEN_RADIUS_DIFF = 4d;

    private static final int CROSSHAIR_ADDL_PIXELS = 7;

    private Button configLandIdCheckbox;

    public UnknownPlotter() {
        super(new UnknownPlotConfig());
    }

    private UnknownPlotConfig getUnknownPlotConfig() {
        return (UnknownPlotConfig) getPlotConfig();
    }
    ...
    public void plot(IGraphicsTarget target, PaintProperties paintProps,
        ...
        try {
             ...
             * depths, etc
             */
            UnknownPlotConfig landPlotConfig = getUnknownPlotConfig();
            for (IPhysicalEvent event : getPhysicalEvents()) {

                if (!(event.getData() instanceof UnknownEventData)) {
                    continue;
                }
                UnknownEventData unkData = (UnknownEventData) event.getData();
                ...
                if (landPlotConfig.isShowCustomId()) {
                    /* Event Id */
                ...
                if (getUnknownPlotConfig().isShowRangeRing()
                        && isEventSelected) {
                    RangeRingRenderer rangeRenderer = new RangeRingRenderer(
                            getUnknownPlotConfig().getRangeRingRadiusKm(),
                            screenLoc);
    ...
    public void buildPlotterConfigTabItems(TabFolder tabFolder) {
        ...
        allButton.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                getUnknownPlotConfig().setShowAll();
                reinitConfigWidgets();
            }
        });
        ...
        noneButton.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                getUnknownPlotConfig().setShowNone();
                reinitConfigWidgets();
            }
        });
