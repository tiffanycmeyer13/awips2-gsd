...
/**
 * Displays a Unknown event data in detail. It's not editable but keeps editable
 * option for future upload a event.
 *
 ...
 * ------------ ---------- ----------------- --------------------------
 * Apr 19, 2922        Jing                  Initial implement Unknown data display tab
 *
 *
 * </pre>
 *
 ...
 *
 */
public class UnknownDataTab extends BasePEMDialogTab {
    private final static boolean IS_EDITABLE = true;

    public UnknownDataTab() {
        super();
    }
    ...
    protected void reinitializeTabContent() {
        ...
        if (getPhysicalEvent() == null) {
            ...
            if (!(data instanceof UnknownEventData)) {
                Label badDataLabel = new Label(getTabContentComposite(),
                        SWT.NONE);
                badDataLabel.setText(
                        "ERROR - Physical Event's Data is not a \"UnknownEventData\"");
            } else {
                UnknownEventData volcanicData = (UnknownEventData) data;
                createUnknownEventDataTab(volcanicData);
            }
        }
    }
     ...
     */
    private void createUnknownEventDataTab(UnknownEventData unkData) {
        ...
        initializeOriginGroup(originGroup, unkData);
    }
