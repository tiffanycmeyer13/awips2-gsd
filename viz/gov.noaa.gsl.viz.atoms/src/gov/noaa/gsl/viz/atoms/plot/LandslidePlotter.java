...
/**
 * Plotter / renderer for Landslide physical events.
 *
 * <pre>
 *
 * SOFTWARE HISTORY
 ...
 *
 */
public class LandslidePlotter extends GeneralPEPlotter {
    ...
    private static final RGB DOT_COLOR = new RGB(255, 255, 0);

    private static final double SELECT_EVENT_CIRCLE_SCREEN_RADIUS_DIFF = 4d;

    private static final int CROSSHAIR_ADDL_PIXELS = 7;

    public LandslidePlotter() {
        super(new LandslidePlotConfig());
    }

    private LandslidePlotConfig getLandPlotConfig() {
        return (LandslidePlotConfig) getPlotConfig();
    }
    ...
    public void plot(IGraphicsTarget target, PaintProperties paintProps,
        ...
        try {
             ...
             * depths, etc
             */
            LandslidePlotConfig landPlotConfig = getLandPlotConfig();
            for (IPhysicalEvent event : getPhysicalEvents()) {

                if (!(event.getData() instanceof LandslideEventData)) {
                    continue;
                }
                LandslideEventData landData = (LandslideEventData) event
                        .getData();
                ...
                if (getLandPlotConfig().isShowRangeRing() && isEventSelected) {
                    RangeRingRenderer rangeRenderer = new RangeRingRenderer(
                            getLandPlotConfig().getRangeRingRadiusKm(),
                            screenLoc);
