...
/**
 * Displays a Landslide event data in detail. It's not editable but keeps
 ...
 * ------------ ---------- ----------------- --------------------------
 * Apr 19, 2922        Jing                  Initial implement Landslide data display tab
 *
 *
 * </pre>
 *
 ...
 *
 */
public class LandslideDataTab extends BasePEMDialogTab {
    private final static boolean IS_EDITABLE = true;

    public LandslideDataTab() {
        super();
    }
    ...
    protected void reinitializeTabContent() {
        ...
        if (getPhysicalEvent() == null) {
            ...
            if (!(data instanceof LandslideEventData)) {
                Label badDataLabel = new Label(getTabContentComposite(),
                        SWT.NONE);
                badDataLabel.setText(
                        "ERROR - Physical Event's Data is not a \"LandslideEventData\"");
            } else {
                LandslideEventData volcanicData = (LandslideEventData) data;
                createLandslideEventDataTab(volcanicData);
            }
        }
    }

    /**
     * Display the landslideData event data.
     *
     * @param landslideData
     *            The landslideData event data to be displayed.
     */
    private void createLandslideEventDataTab(LandslideEventData landslideData) {
        ...
        initializeOriginGroup(originGroup, landslideData);
    }
    ...
    private void initializeOriginGroup(Group originGroup,
            LandslideEventData landslideData) {
        if (getPhysicalEvent() == null) {
            return;
        }
