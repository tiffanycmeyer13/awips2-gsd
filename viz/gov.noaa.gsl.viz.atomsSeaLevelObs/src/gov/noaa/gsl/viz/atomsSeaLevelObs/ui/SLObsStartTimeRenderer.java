 ...
 * @version 1.0
 */
public class SLObsStartTimeRenderer {

    private SeaLevelObs slob;

    private Capabilities drawCapabilities = null;

    public SLObsStartTimeRenderer(SeaLevelObs slob) {
        if (slob == null) {
            return;
        }
        this.slob = slob;
    ...
    public void plot(IGraphicsTarget target, IMapDescriptor descriptor,
        ...
        Date startTime = slob.getStartTime();
        String startTimeString = "N/A";
        if (startTime != null) {
            startTimeString = PemUtils.getHourMinuteSecFormatter()
                    .format(startTime);
        }
        DrawableString plot = new DrawableString(startTimeString + " ", color);
