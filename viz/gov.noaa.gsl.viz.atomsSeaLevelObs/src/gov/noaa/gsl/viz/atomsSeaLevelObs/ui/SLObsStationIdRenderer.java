 ...
 * @version 1.0
 */
public class SLObsStationIdRenderer {

    private SeaLevelObs slob;

    private Capabilities drawCapabilities = null;

    public SLObsStationIdRenderer(SeaLevelObs slob) {
        if (slob == null) {
            return;
        }
        this.slob = slob;
    ...
    public void plot(IGraphicsTarget target, IMapDescriptor descriptor,
        ...
        /* Station name */
        String stationId = slob.getStation().getCustomId();
        DrawableString name = new DrawableString(stationId + " ", color);
